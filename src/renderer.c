...
double ren_font_group_get_width(
    RenFont** fonts,
    const char* text,
    size_t len,
    RenTab tab,
...
static unsigned int font_get_glyph_id(RenFont* font, unsigned int codepoint) {
    ...
    if (!font->charmap.rows[row])
        font->charmap.rows[row] = check_alloc(SDL_calloc(sizeof(unsigned int), CHARMAP_COL));
...
static GlyphMetric* font_load_glyph_metric(RenFont* font,
    ...
    unsigned int bitmap_idx)
{
    ...
    // Only now compute expensive stuff
    unsigned int load_option = font_set_load_options(font);

    if (FT_Load_Glyph(font->face, glyph_id,
        (load_option | FT_LOAD_BITMAP_METRICS_ONLY | FT_LOAD_NO_HINTING)
        & ~FT_LOAD_FORCE_AUTOHINT) != 0)
        return NULL;

    const float xadvance = font->face->glyph->advance.x * (1.0f / 64.0f);
    const int bitmaps = FONT_BITMAP_COUNT(font);

    for (int i = 0; i < bitmaps; i++) {
        GlyphMetric** metrics = font->glyphs.metrics[i];

        if (!metrics[row]) {
            metrics[row] = check_alloc(SDL_calloc(GLYPHMAP_COL, sizeof(GlyphMetric)));
            font->glyphs.bytesize += sizeof(GlyphMetric) * GLYPHMAP_COL;
        }

        GlyphMetric* metric = &metrics[row][col];
...
static SDL_Surface *font_load_glyph_bitmap(RenFont *font, unsigned int glyph_id, unsigned int bitmap_idx) {
  ...
  if (metric->flags & EGlyphBitmap) {
      if (metric->x1 == 0 || metric->y1 == 0)
          return NULL;

      return font->glyphs.atlas[metric->format]
          [metric->atlas_idx].surfaces[metric->surface_idx];
  }

  printf("Rendering glyph %u bitmap_idx=%u\n", glyph_id, bitmap_idx);
...
RenFont* ren_font_load(const char* path, float size, ERenFontAntialiasing antialiasing, ERenFontHinting hinting, unsigned char style) {
  ...
  font = check_alloc(SDL_calloc(1, sizeof(RenFont) + len));

  memcpy(font->path, path, len);
  font->size = size;
...
//
//    // hoist format details out of the inner loop — they don't change per glyph
//    const SDL_PixelFormatDetails* surface_format = SDL_GetPixelFormatDetails(surface->format);
//
//    // minimum advance per character — used to fast-skip left-clipped characters
//    // space_advance is a safe lower bound for most fonts
//    float min_advance = fonts[0]->space_advance * surface_scale;
//
//    while (text < end) {
//        // fast skip: if even the rightmost possible pixel of the next N characters
//        // cannot reach clip.x, skip without doing any glyph lookup at all.
//        // we use min_advance as a conservative lower bound on advance width.
//        if (min_advance > 0 && pen_x + min_advance < clip.x) {
//            // count how many chars we can skip entirely
//            double deficit = clip.x - pen_x;
//            int skip_chars = (int)(deficit / min_advance) - 1; // -1 for safety margin
//            if (skip_chars > 1) {
//                // advance through the utf8 string by skip_chars codepoints
//                for (int i = 0; i < skip_chars && text < end; i++) {
//                    unsigned int cp;
//                    const char* next = utf8_to_codepoint(text, end, &cp);
//                    // still need accurate advance for tab chars, bail out of fast skip
//                    if (cp == '\t') break;
//                    text = next;
//                    // use space_advance as an approximation — we'll correct when we
//                    // get close to the clip boundary with real glyph lookups
//                    pen_x += min_advance;
//                }
//            }
//        }
...
//        int end_x = metric->x1 + start_x;
//        int glyph_end = metric->x1, glyph_start = 0;
//
//        float adv = font_get_xadvance(fonts[0], codepoint, metric, pen_x - original_pen_x, tab);
//
//        // break early — everything to the right is also off screen
//        if (start_x >= clip_end_x) {
//            pen_x += adv;
//            if (last && (underline || strikethrough)) {
//                double local_pen_x = pen_x;
//                if (underline)
...
//            // compute clip adjustments once per glyph, not per scanline
//            int glyph_start_clipped = glyph_start;
//            int glyph_end_clipped = glyph_end;
//            int start_x_clipped = start_x;
//            if (start_x_clipped < clip.x) {
//                int offset = clip.x - start_x_clipped;
//                start_x_clipped += offset;
//                glyph_start_clipped += offset;
//            }
//            if (start_x_clipped + (glyph_end_clipped - glyph_start_clipped) >= clip_end_x)
//                glyph_end_clipped = glyph_start_clipped + (clip_end_x - start_x_clipped);
//
//            const SDL_PixelFormatDetails* font_surface_format = SDL_GetPixelFormatDetails(font_surface->format);
//            bool is_subpixel = metric->format == EGlyphFormatSubpixel;
//
...
//
//                uint32_t* destination_pixel = (uint32_t*)&(destination_pixels[surface->pitch * target_y + start_x_clipped * surface_format->bytes_per_pixel]);
//                uint8_t* source_pixel = &source_pixels[line * font_surface->pitch + glyph_start_clipped * font_surface_format->bytes_per_pixel];
//
//                for (int x = glyph_start_clipped; x < glyph_end_clipped; ++x) {
...
static uint32_t fast_hash_partial(const char* data, size_t len)
{
    uint32_t h = (uint32_t)len;

    if (len > 0) {
        h ^= (unsigned char)data[0];
        h ^= (unsigned char)data[len - 1] << 8;
        h ^= (unsigned char)data[len / 2] << 16;
    }

    return h;
}
static uint32_t fast_hash(const char* data, size_t len)
{
...
typedef struct RenLayoutCacheEntry {
    ...
    struct RenLayoutCacheEntry* prev;
    struct RenLayoutCacheEntry* next;
} RenLayoutCacheEntry;

static RenLayoutCacheEntry layout_cache[LAYOUT_CACHE_SIZE];

static RenLayoutCacheEntry* lru_head = NULL;
static RenLayoutCacheEntry* lru_tail = NULL;
static void lru_insert_front(RenLayoutCacheEntry* e)
{
    e->prev = NULL;
    e->next = lru_head;
...
static void lru_remove(RenLayoutCacheEntry* e)
{
    if (e->prev) e->prev->next = e->next;
    if (e->next) e->next->prev = e->prev;

    if (lru_head == e) lru_head = e->next;
    if (lru_tail == e) lru_tail = e->prev;

    e->prev = e->next = NULL;
}
static void lru_touch(RenLayoutCacheEntry* e)
{
    if (lru_head == e)
        return;
...
static RenLayoutCacheEntry* get_cache_entry(
    uint32_t hash,
    size_t len,
    RenFont** fonts,
    RenTab tab)
{
    for (size_t i = 0; i < LAYOUT_CACHE_SIZE; i++) {
        RenLayoutCacheEntry* e = &layout_cache[i];
    ...
    RenLayoutCacheEntry* victim = NULL;

    for (size_t i = 0; i < LAYOUT_CACHE_SIZE; i++) {
        if (!layout_cache[i].valid) {
            victim = &layout_cache[i];
            break;
        }
    }
    ...
    RenTab tab)
{
    ...
    //printf("len=%zu\n", len);
    //printf(text); printf("\n");
    //printf("%d  layout rebuild: tab.offset=%f\n", rand(), tab.offset);
    lc->count = 0;
    double pen_x = 0;
    const char* end = text + len;
    ...
    while (text < end) {
        ...
        if (lc->count >= lc->capacity) {
            lc->capacity = lc->capacity ? lc->capacity * 2 : 64;
            lc->metrics = SDL_realloc(lc->metrics,
                lc->capacity * sizeof(GlyphMetric*));
            lc->surfaces = SDL_realloc(lc->surfaces,
                lc->capacity * sizeof(SDL_Surface*));
            lc->positions = SDL_realloc(lc->positions,
                lc->capacity * sizeof(float));
        }

        lc->metrics[lc->count] = metric;
        lc->surfaces[lc->count] = surface;
        lc->positions[lc->count] = pen_x;
...
void ren_layout_text(
    RenTextLayout* layout,
    RenFont** fonts,
    const char* text,
    size_t len,
    RenTab tab)
{
    ...
    while (text < end) {
        ...
        if (layout->count >= layout->capacity) {
            layout->capacity = layout->capacity ? layout->capacity * 2 : 64;
            layout->glyphs = SDL_realloc(layout->glyphs,
                layout->capacity * sizeof(RenPositionedGlyph));
        }

        layout->glyphs[layout->count++] = (RenPositionedGlyph){
          .metric = metric,
          .surface = surface,
          .x = pen_x,
          .codepoint = codepoint
    ...
    RenColor color)
{
    ...
    for (size_t i = 0; i < layout->count; i++) {
        RenPositionedGlyph* g = &layout->glyphs[i];
        GlyphMetric* metric = g->metric;
        SDL_Surface* font_surface = g->surface;

        if (!metric || !font_surface || color.a == 0)
            continue;

        double pen_x = (x + g->x) * surface_scale;

        int start_x = floor(pen_x) + metric->bitmap_left;
        int glyph_start = 0;
        int glyph_end = metric->x1;
        int end_x = start_x + metric->x1;

        if (end_x < clip.x || start_x >= clip_end_x)
            continue;
        ...
        uint8_t* source_pixels = font_surface->pixels;
        const SDL_PixelFormatDetails* font_surface_format =
            SDL_GetPixelFormatDetails(font_surface->format);

        bool is_subpixel = metric->format == EGlyphFormatSubpixel;

        for (int line = metric->y0; line < metric->y1; ++line) {
            int target_y = line - metric->y0
                + y
                - metric->bitmap_top
            ...
            uint32_t* destination_pixel =
                (uint32_t*)&destination_pixels[
                    surface->pitch * target_y +
                        start_x * surface_format->bytes_per_pixel];

            uint8_t* source_pixel =
                &source_pixels[
                    line * font_surface->pitch +
                        glyph_start * font_surface_format->bytes_per_pixel];

            for (int xpix = glyph_start; xpix < glyph_end; ++xpix) {
                uint32_t destination_color = *destination_pixel;
                ...
                unsigned int r =
                    (color.r * src.r * color.a +
                        dst.r * (65025 - src.r * color.a) + 32767) / 65025;

                unsigned int gcol =
                    (color.g * src.g * color.a +
                        dst.g * (65025 - src.g * color.a) + 32767) / 65025;

                unsigned int b =
                    (color.b * src.b * color.a +
                        dst.b * (65025 - src.b * color.a) + 32767) / 65025;

                *destination_pixel++ =
                    (unsigned int)dst.a << surface_format->Ashift |
                    r << surface_format->Rshift |
                    gcol << surface_format->Gshift |
                    b << surface_format->Bshift;
            }
        }
    }
}
static void ren_draw_internal_layout(
    RenSurface* rs,
    RenFont** fonts,
    RenInternalLayout* lc,
    float x,
    ...
    RenColor color)
{
    ...
    y *= surface_scale;
    double clip_left = clip.x / (double)surface_scale - x;
    double clip_right = clip_end_x / (double)surface_scale - x;
    size_t start = 0;
    size_t end = lc->count;

    // find first glyph whose right edge >= clip_left
    size_t lo = 0, hi = lc->count;
    while (lo < hi) {
        size_t mid = (lo + hi) / 2;
        if (lc->positions[mid] + lc->metrics[mid]->x1 < clip_left)
            lo = mid + 1;
        else
            hi = mid;
    }
    ...
    for (size_t i = start; i < lc->count; i++) {
        visible_count++;
        if (lc->positions[i] > clip_right)
            break;
        GlyphMetric* metric = lc->metrics[i];
        SDL_Surface* font_surface = lc->surfaces[i];

        if (!metric || !font_surface || color.a == 0)
            continue;

        double pen_x = (x + lc->positions[i]) * surface_scale;
        if (pen_x > 10000 || pen_x < -10000) {
            printf("pen: %d\n", pen_x);
        }
        int start_x = floor(pen_x) + metric->bitmap_left;
        int glyph_start = 0;
    ...
    //printf("processed glyphs: %zu (total: %zu)\n", visible_count, lc->count);
}
...
double ren_font_group_get_width(
    ...
    /* Handle x_offset */
    if (x_offset) {
        if (entry->layout.count > 0 &&
            entry->layout.metrics[0])
        {
            *x_offset = entry->layout.metrics[0]->bitmap_left;
        }
        else {
            *x_offset = 0;
        }
...
#ifdef LITE_USE_SDL_RENDERER
    return entry->layout.width / fonts[0]->scale;
#else
    return entry->layout.width;
#endif
}
