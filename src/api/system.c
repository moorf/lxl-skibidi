...
#define KEYNAME_MAX 64  /* safely larger than any SDL key name */
...
static int f_chdir(lua_State* L) {
    ...
    int err = chdir(path);
#endif
    if (err) { char errbuf[256]; strerror_s(errbuf, sizeof(errbuf), errno); luaL_error(L, "chdir() failed: %s", errbuf); }
    return 0;
}
...
#error luaL_Stream is not supported in this version of Lua.
    ...
    if (ftruncate(fileno(stream->f), len) != 0) {
        lua_pushboolean(L, 0);
        char errbuf[256]; strerror_s(errbuf, sizeof(errbuf), errno);
        lua_pushfstring(L, "ftruncate(): %s", errbuf);
        return 2;
    }
...
static int f_exec(lua_State* L) {
    ...
    /* Worst case size:
       Windows: "cmd /c \"\""
       Unix: " &"
       So allocate len + 16 safely */
    size_t bufsize = len + 16;
    char* buf = SDL_malloc(bufsize);
    if (!buf) {
