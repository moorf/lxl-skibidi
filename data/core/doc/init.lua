...
function Doc:new(filename, abs_filename, new_file)
  ...
  --print(info.short_src, info.currentline, info.name)
...
function Doc:load(filename)
  ...
  self:load_chunk(100) -- load first 100 lines
  self:reset_syntax()
end

function Doc:load_chunk(count)
...
function Doc:draw()
  local first = self.scroll.y
  local last = first + self.visible_lines

  -- Load more if near bottom
  if not self.eof and last > #self.lines - 20 then
    self:load_chunk(100)
  end

  for i = first, math.min(last, #self.lines) do
    self:draw_line(i)
  end
end
...
function Doc:save(filename, abs_filename)
  ...
  -- ✅ Ensure the whole file is loaded before saving
  if not self.eof then
    while not self.eof do
      self:load_chunk(1000)
    end
  end
  ...
  -- ✅ Now write entire document (fully loaded)
