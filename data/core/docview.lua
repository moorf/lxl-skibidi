...
function DocView:get_x_offset_col(line, target_x)
  ...
  local last_col = #columns
  if target_x >= columns[last_col] then
    return last_col
  end

  -- Binary search
  local low, high = 1, last_col
  ...
  -- Midpoint snapping (matches original behavior)
...
function DocView:get_x_offset_cold(line, target_x)
  self._layout_cache = self._layout_cache or {}
